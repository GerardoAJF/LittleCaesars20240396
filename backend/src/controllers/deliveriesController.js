...
deliveriesController.insertDeliveries = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Delivery saved" });
  } catch (error) {
    console.log("error: " + error);
    return res.status(500).json({ message: "Internal server error" });
  }
...
deliveriesController.updateDeliveries = async (req, res) => {
    try {
        ...
        // Identificar que repartidos voy a actualizar
        const deliveryFound = await deliveriesModel.findById(req.params.id)
        ...
        const deliveryUpdated = await deliveriesModel.findByIdAndUpdate(
            req.params.id, 
            updatedData, 
